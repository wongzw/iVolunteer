<template>
  <div class="box" v-if="this.eventLoaded">
      ...
      <a-form-item>
        ...
        <a-modal v-model:visible="visible" title="Confirm Deletion">
          ...
          <div class="modal">
            ...
            <p>
              <strong
                >By clicking confirm, you will be deleting <br /><br />
                <b>{{ this.eventName }} </b></strong
              >
            </p>
        ...
        ></a-input
        ><br /><br />
        <label class="eventEditLeft">Event causes (Max: 3)</label><br /><br />
        <a-select
    ...
    <div id="delete-button">
      <a-button class="signUp" size="large" danger block @click="clickDelete"
        >Delete Event
      </a-button>
    </div>
  </div>
  ...
  methods: {
    ...
    async deleteEvent() {
      ...
      for (var participant in this.participants) {
        const volRef = doc(db, "users", participant);
        await updateDoc(volRef, {
          userNotification: arrayUnion(newNotification),
        });
        this.updateRejectEvent(participant);
      }
    },

    async updateRejectEvent(participant) {
      let participantId = participant;
      const eventDocRef = doc(db, "events", this.id);
